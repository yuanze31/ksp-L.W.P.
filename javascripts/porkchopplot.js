...
(function () {
  ...
  PorkchopPlot = (function () {
    ...
    function PorkchopPlot(_at_container) {
      ...
      $(".zoomInBtn", this.container).click(
        (function (_this) {
          return function (event) {
            return _this.zoomIn();
          };
      ...
      $(".zoomOutBtn", this.container).click(
        (function (_this) {
          return function (event) {
          ...
          (function (_this) {
            return function (event) {
              ...
              if (_this.deltaVs != null && _this.dragStart == null) {
                offsetX =
                  ((_ref = event.offsetX) != null
                    ? _ref
                    : event.pageX - _this.canvas.offset().left) | 0;
                offsetY =
                  ((_ref1 = event.offsetY) != null
                    ? _ref1
                    : event.pageY - _this.canvas.offset().top) | 0;
    ...
    workerMessage = function (event) {
      ...
      if ("log" in event.data) {
        ...
        return this.progressContainer
          .show()
          .find(".progress-bar")
          .width(((event.data.progress * 100) | 0) + "%");
        ...
        for (
          y = _n = 0;
          0 <= PLOT_HEIGHT ? _n < PLOT_HEIGHT : _n > PLOT_HEIGHT;
          y = 0 <= PLOT_HEIGHT ? ++_n : --_n
        ) {
          for (
            x = _o = 0;
            0 <= PLOT_WIDTH ? _o < PLOT_WIDTH : _o > PLOT_WIDTH;
            x = 0 <= PLOT_WIDTH ? ++_o : --_o
          ) {
            logDeltaV = Math.log(this.deltaVs[i++]);
            if (isNaN(logDeltaV)) {
              color = [255, 255, 255];
            } else {
              relativeDeltaV = isNaN(logDeltaV)
                ? 1.0
                : (logDeltaV - logMinDeltaV) / (logMaxDeltaV - logMinDeltaV);
    ...
    prepareCanvas = function () {
      ...
      for (
        y = _p = 0;
        0 <= PLOT_HEIGHT ? _p < PLOT_HEIGHT : _p > PLOT_HEIGHT;
        y = 0 <= PLOT_HEIGHT ? ++_p : --_p
      ) {
      ...
      ctx.fillText(
        String.fromCharCode(0x2206) + "v",
        PLOT_X_OFFSET + PLOT_WIDTH + 45,
        PLOT_HEIGHT / 2
      );
      return ctx.restore();
    };
    ...
    PorkchopPlot.prototype.drawAxisLabels = function () {
      ...
      for (i = _n = 0; _n <= 1.0; i = _n += 0.25) {
        ...
        ctx.fillText(
          ((this.mission.shortestTimeOfFlight + i * this.mission.yScale) /
            KerbalTime.secondsPerDay()) |
            0,
          PLOT_X_OFFSET - TIC_LENGTH - 3,
          (1.0 - i) * PLOT_HEIGHT
        );
      }
      ctx.textAlign = "center";
      for (i = _o = 0; _o <= 1.0; i = _o += 0.25) {
        ctx.fillText(
          ((this.mission.earliestDeparture + i * this.mission.xScale) /
            KerbalTime.secondsPerDay()) |
            0,
          PLOT_X_OFFSET + i * PLOT_WIDTH,
          PLOT_HEIGHT + TIC_LENGTH + 3
        );
      }
      return ctx.restore();
    };
    ...
    startPanning = function (pageX, pageY, touchIdentifier) {
      ...
      if (this.deltaVs != null) {
        ...
        if (
          offsetX >= PLOT_X_OFFSET &&
          offsetX < PLOT_X_OFFSET + PLOT_WIDTH &&
          offsetY < PLOT_HEIGHT
        ) {
    ...
    stopPanning = function (pageX, pageY, showPointer) {
      ...
      if (this.dragged) {
        if (this.dragStart.x !== pageX || this.dragStart.y !== pageY) {
          deltaX = pageX - this.dragStart.x;
          deltaY = pageY - this.dragStart.y;
          this.mission.earliestDeparture = Math.max(
            this.mission.earliestDeparture - deltaX * this.mission.xResolution,
            0
          );
          this.mission.shortestTimeOfFlight = Math.max(
            this.mission.shortestTimeOfFlight + deltaY * this.mission.yResolution,
            1
          );
        ...
        if (
          ...
          x < PLOT_WIDTH &&
          y < PLOT_HEIGHT &&
          !isNaN(this.deltaVs[(y * PLOT_WIDTH + x) | 0])
        ) {
          this.selectedPoint = {
            x: x,
            y: PLOT_HEIGHT - 1 - y,
