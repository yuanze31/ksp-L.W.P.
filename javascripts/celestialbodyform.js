...
(function () {
  ...
  CelestialBodyForm = (function () {
    function CelestialBodyForm(_at_form) {
      this.form = _at_form;
      $("#bodyType a", this.form).click(
        (function (_this) {
          return function (event) {
            event.preventDefault();
            $(event.target).tab("show");
            return $("#bodySaveBtn", _this.form).prop(
              "disabled",
              $("#bodyForm .form-group.has-error:visible").length > 0
            );
          };
        })(this)
      );
      $("#bodySaveBtn", this.form).click(
        (function (_this) {
          return function (event) {
            return _this.save();
          };
      ...
      $("#semiMajorAxis,#planetMass,#planetRadius").blur(
        (function (_this) {
          return function (event) {
      ...
      $("#longitudeOfAscendingNode,#argumentOfPeriapsis", this.form).blur(
        (function (_this) {
          return function (event) {
      ...
      $("#meanAnomalyAtEpoch", this.form).blur(
        (function (_this) {
          return function (event) {
      ...
      $("#timeOfPeriapsisPassage", this.form).blur(
        (function (_this) {
          return function (event) {
    ...
    CelestialBodyForm.prototype.add = function (referenceBody, _at_callback) {
      ...
      if (referenceBody != null) {
        ...
        $(".modal-header h4", this.form).text("以" + referenceBody.name() + "为基准新建物体");
      } else {
        $("#referenceBodySelect", this.form).val("Kerbol").prop("disabled", false);
        $(".modal-header h4", this.form).text("新建物体");
      }
      $("#bodyName", this.form).val("").removeData("originalValue");
      $(
        "#semiMajorAxis,#eccentricity,#inclination,#longitudeOfAscendingNode,#argumentOfPeriapsis,#meanAnomalyAtEpoch,#planetMass,#planetRadius,#timeOfPeriapsisPassage",
        this.form
      ).val("");
    ...
    CelestialBodyForm.prototype.edit = function (body, fixedReferenceBody, _at_callback) {
      ...
      if (body.mass != null) {
        ...
        $("#timeOfPeriapsisPassage", this.form).val(
          new KerbalTime(orbit.timeOfPeriapsisPassage).toShortDateString()
        );
      }
      $(".modal-header h4", this.form).text("编辑" + body.name());
      $("#bodyName", this.form).val(body.name()).data("originalValue", body.name());
      $("#referenceBodySelect", this.form)
        .val(body.orbit.referenceBody.name())
        .prop("disabled", fixedReferenceBody);
      ...
      $("#longitudeOfAscendingNode", this.form).val(
        (orbit.longitudeOfAscendingNode * 180) / Math.PI
      );
    ...
    CelestialBodyForm.prototype.validateEccentricity = function (input) {
      ...
      if (isNaN(val) || isBlank(val)) {
        ...
        $input
          ...
          .addClass("has-error")
          .find(".help-block")
          .text("Must be between 0 and 1 (hyperbolic orbits are not supported)")
          .show();
      } else {
        $input.closest(".form-group").removeClass("has-error").find(".help-block").hide();
      }
    ...
    CelestialBodyForm.prototype.validateAngle = function (input, maxAngle) {
      ...
      if (isNaN(val) || isBlank(val)) {
        ...
        $input
          ...
          .addClass("has-error")
          .find(".help-block")
          .text("Must be between 0\u00B0 and " + maxAngle + "\u00B0")
          .show();
      } else {
        $input.closest(".form-group").removeClass("has-error").find(".help-block").hide();
      }
    ...
    CelestialBodyForm.prototype.validateMeanAnomaly = function (input) {
      ...
      if (isNaN(val) || isBlank(val)) {
        ...
        $input
          ...
          .addClass("has-error")
          .find(".help-block")
          .text("Must be between 0 and 2\u03c0 (6.28\u2026)")
          .show();
      } else {
        $input.closest(".form-group").removeClass("has-error").find(".help-block").hide();
      }
    ...
    CelestialBodyForm.prototype.validateDate = function (input) {
      ...
      if (isBlank(val)) {
        ...
        $input
          ...
          .addClass("has-error")
          .find(".help-block")
          .text("Must be a valid Kerbal date: year/day hour:min:sec")
          .show();
      } else {
        $input.closest(".form-group").removeClass("has-error").find(".help-block").hide();
      }
  ...
  (typeof exports !== "undefined" && exports !== null ? exports : this).CelestialBodyForm =
    CelestialBodyForm;
}).call(this);
