...
(function () {
  ...
  (typeof exports !== "undefined" && exports !== null ? exports : this).prepareOrigins =
    prepareOrigins = function (selectedOrigin) {
      var addPlanetGroup,
        bodies,
        body,
        listBody,
  ...
  MissionForm = (function () {
    ...
    function MissionForm(_at_form, _at_celestialBodyForm) {
        ...
        (function (_this) {
          return function (event, oldHoursPerDay, oldDaysPerYear) {
            ...
            for (_i = 0, _len = departureDates.length; _i < _len; _i++) {
              ...
              oldHours =
                ((+$("#" + departureDate + "DepartureYear").val() - 1) * oldDaysPerYear +
                  (+$("#" + departureDate + "DepartureDay").val() - 1)) *
                oldHoursPerDay;
            ...
            return $("#longestTimeOfFlight").val(
              +(oldMaxHours / KerbalTime.hoursPerDay).toFixed(2)
            );
          };
        })(this)
      );
      ...
      $("#noInsertionBurnCheckbox").change(
        (function (_this) {
          return function (event) {
      ...
      $("#earliestDepartureYear,#earliestDepartureDay").change(
        (function (_this) {
          return function (event) {
      ...
      $("#shortestTimeOfFlight,#longestTimeOfFlight").change(function (event) {
        return setTimeOfFlight(
          +$("#shortestTimeOfFlight").val(),
          +$("#longestTimeOfFlight").val(),
          event.target.id === "shortestTimeOfFlight"
        );
      });
      this.form.bind(
        "reset",
    ...
    MissionForm.prototype.advancedControlsVisible = function () {
      return $("#showAdvancedControls").text().indexOf("隐藏") !== -1;
    };
    ...
    MissionForm.prototype.mission = function () {
      ...
      earliestDeparture = KerbalTime.fromDate(
        +$("#earliestDepartureYear").val(),
        +$("#earliestDepartureDay").val()
      ).t;
      latestDeparture = KerbalTime.fromDate(
        +$("#latestDepartureYear").val(),
        +$("#latestDepartureDay").val()
      ).t;
      xScale = latestDeparture - earliestDeparture;
      shortestTimeOfFlight = KerbalTime.fromDuration(0, +$("#shortestTimeOfFlight").val()).t;
      yScale =
        KerbalTime.fromDuration(0, +$("#longestTimeOfFlight").val()).t - shortestTimeOfFlight;
      params = [
        $("#originSelect").val(),
        initialOrbit,
    ...
    updateAdvancedControls = function () {
      ...
      destination = this.destination();
      referenceBody = origin.orbit.referenceBody;
      hohmannTransfer = Orbit.fromApoapsisAndPeriapsis(
        referenceBody,
      ...
      departureRange =
        Math.min(2 * synodicPeriod, 2 * origin.orbit.period()) / KerbalTime.secondsPerDay();
      ...
      minDeparture =
        KerbalTime.fromDate($("#earliestDepartureYear").val(), $("#earliestDepartureDay").val()).t /
        KerbalTime.secondsPerDay();
      maxDeparture = minDeparture + departureRange;
      minDays =
        Math.max(hohmannTransferTime - destination.orbit.period(), hohmannTransferTime / 2) /
        KerbalTime.secondsPerDay();
      maxDays =
        minDays +
        Math.min(2 * destination.orbit.period(), hohmannTransferTime) / KerbalTime.secondsPerDay();
