...
(function () {
  ...
  deltaVAbbr = function (el, dv, prograde, normal, radial) {
    ...
    tooltip =
      numberWithCommas(prograde.toFixed(1)) +
      " m/s prograde; " +
      numberWithCommas(normal.toFixed(1)) +
      " m/s normal";
    if (radial != null) {
  ...
  showTransferDetailsForPoint = function (point) {
    ...
    transfer = Orbit.transfer(
      mission.transferType,
      mission.originBody,
      mission.destinationBody,
      t0,
      dt,
      mission.initialOrbitalVelocity,
      mission.finalOrbitalVelocity
    );
  ...
  showTransferDetails = function (transfer, t0, dt) {
    ...
    mission = porkchopPlot.mission;
    t1 = t0 + dt;
    transfer = Orbit.transferDetails(
      transfer,
      mission.originBody,
      t0,
      mission.initialOrbitalVelocity
    ...
    $("#departureTime")
      .text(new KerbalTime(t0).toDateString())
      .attr({
        title: "UT: " + t0.toFixed() + "s",
      });
    $("#arrivalTime")
      .text(new KerbalTime(t1).toDateString())
      .attr({
        title: "UT: " + t1.toFixed() + "s",
      });
    $("#timeOfFlight")
      .text(new KerbalTime(dt).toDurationString())
      .attr({
        title: dt.toFixed() + "s",
      });
    ...
    if (transfer.planeChangeTime != null) {
      ...
      $("#planeChangeTime")
        .text(new KerbalTime(transfer.planeChangeTime).toDateString())
        .attr({
      ...
      deltaVAbbr(
        $("#planeChangeDeltaV"),
        transfer.planeChangeDeltaV,
        -transfer.planeChangeDeltaV * Math.abs(Math.sin(transfer.planeChangeAngle / 2)),
        transfer.planeChangeDeltaV *
          sign(transfer.planeChangeAngle) *
          Math.cos(transfer.planeChangeAngle / 2)
      );
    } else {
  ...
  ejectionDeltaVInfoContent = function () {
    ...
    $("<dd>")
      .text(numberWithCommas(selectedTransfer.ejectionProgradeDeltaV.toFixed(1)) + " m/s")
      .appendTo(list);
    $("<dt>").text("法向\u0394v").appendTo(list);
    $("<dd>")
      .text(numberWithCommas(selectedTransfer.ejectionNormalDeltaV.toFixed(1)) + " m/s")
      .appendTo(list);
    if (selectedTransfer.ejectionRadialDeltaV != null) {
      $("<dt>").text("径向\u0394v").appendTo(list);
      $("<dd>")
        .text(numberWithCommas(selectedTransfer.ejectionRadialDeltaV.toFixed(1)) + " m/s")
        .appendTo(list);
    }
  ...
  $(document).ready(function () {
    ...
    $(missionForm).on("submit", function (event) {
      ...
      scrollTop =
        $("#porkchopCanvas").offset().top + $("#porkchopCanvas").height() - $(window).height();
    ...
    return $("#refineTransferBtn").click(function (event) {
      ...
      (_ref = [porkchopPlot.selectedPoint.x, porkchopPlot.selectedPoint.y]),
        (x = _ref[0]),
        (y = _ref[1]);
      mission = porkchopPlot.mission;
      t0 = mission.earliestDeparture + x * mission.xResolution;
      dt = mission.shortestTimeOfFlight + y * mission.yResolution;
      transfer = Orbit.refineTransfer(
        selectedTransfer,
        mission.transferType,
        mission.originBody,
        mission.destinationBody,
